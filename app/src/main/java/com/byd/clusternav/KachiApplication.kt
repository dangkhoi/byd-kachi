...
class KachiApplication : Application() {
    override fun onCreate() {
        ...
        // (`:tts`, chỉ chứa [PiperTtsService]). Không có cổng này thì `:tts` cũng nạp sẵn mô hình NGHE (74 MB
        // int8) + Silero VAD: vừa vô ích (nó chỉ ĐỌC, không nghe) vừa đúng thứ gây ra chính lỗi đang vá — SIGSEGV
        // của `OfflineTts.generate` là `SEGV_MAPERR` dưới áp lực RAM ([ĐO] xe chạy GMaps + VietMap + cdr). Cô lập
        // tiến trình mà nhân đôi RAM là cô lập hỏng.
        if (isTtsProcess()) return
    ...
    /**
     * Đang chạy trong tiến trình đọc (`:tts`) chứ không phải tiến trình launcher?
     *
     ...
     *
     * So bằng chính [PiperTtsService.PROCESS_SUFFIX] để manifest và mã không thể lệch nhau — bài canh khoá cặp đó.
     */
