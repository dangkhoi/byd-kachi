 ...
 *  2. **Đọc sấy trước** — `carControl.readState("defrost")`, tức đi qua `ControlDef.readKey`
 *     (`defrost_front_state` → `BYDAutoAcDevice.getAcDefrostState(1)`). ⚠ **Phải đọc TỪ XE**, không phải một cờ
 *     RAM: cờ RAM không thấy người lái bấm nút sấy trên màn xe, tức R1.5 chết (xem hợp đồng stage 1).
 *  3. **Ghi sấy** — `carControl.toggle("defrost"/"defrost_rear", on)`, tức đúng cửa mà một cú chạm dùng
 *     (`AC_DEFROST_FRONT_STATE_SET` / `AC_DEFROST_REAR_STATE_SET`, CAPTEST OK). Không tự dựng lệnh HAL thứ hai:
 *     dự án đã trả giá cho đúng việc ấy ở cặp `lock`/`door` (hai nhãn đối nghịch gửi cùng một byte).
 *
...
object RainDefrostApplier {
    ...
    /** Mã nút sấy SAU. Owner chốt bật/tắt **cả hai** (R1.3), không chỉ kính lái. */
    ...
    fun tick(ctx: Context): RainDefrostAction {
        ...
        return runCatching {
            ...
            val defrostOn = readDefrostFront(app)
            if (defrostOn == null) {
                ...
                Log.d(TAG, "sấy: không đọc được trạng thái ⇒ giữ nguyên")
                state = RainDefrostOwner.stepUnknown(state).state
                return@runCatching RainDefrostAction.Leave
            }
            ...
            Log.i(
                TAG,
                "mưa=$speed (mưa? ${RainDefrostPolicy.isRaining(speed)}) · sấy=$defrostOn · " +
    ...
    /** Sấy TRƯỚC đang bật? Đọc **từ xe** qua `readKey` của nút. `null` = chưa đọc được. */
    private fun readDefrostFront(app: Context): Boolean? = runCatching {
        AppContainer.get(app).carControl.readState(CTL_FRONT)?.let { it != 0 }
    }.getOrNull()

    /**
     * Ghi CẢ hai nút sấy (R1.3). Một nút hỏng KHÔNG chặn nút kia — xe từ chối một lệnh là chuyện thường.
     *
     * @return lệnh cho nút **TRƯỚC** đã tới xe chưa. Chỉ nút trước quyết định chủ quyền, vì nó cũng là nút được
     *   ĐỌC lại ở nhịp sau ([CTL_FRONT] → `readKey`); sấy sau hỏng riêng thì automation vẫn theo dõi đúng cái nó
     *   đang giữ, và nhịp sau sẽ không hiểu lầm thành *"người lái tự tắt"*.
     */
    private fun writeBoth(app: Context, on: Boolean): Boolean {
        ...
        var front = false
        listOf(CTL_FRONT, CTL_REAR).forEach { id ->
            val ok = runCatching { control.toggle(id, on) }.getOrDefault(false)
            if (id == CTL_FRONT) front = ok
