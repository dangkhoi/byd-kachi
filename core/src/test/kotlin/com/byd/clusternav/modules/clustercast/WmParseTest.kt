 ...
 *
 * ⚠ Dời từ `:app` sang `:core` ngày 2026-09-13 (`LayeringRulesTest.test cua logic core khong duoc nam trong app`):
 * bài chỉ dùng `WmParse`/`StackParse` — hai parser thuần của `:core`. Trước đó nó "có vẻ" thuộc `:app` chỉ vì
 * fixture `dumpsys` có chuỗi `…/.MainActivity` (tên một lớp của `:app` **lúc đó**), và cái tên đó biến mất khi màn
 * ClusterNav cũ bị gỡ. Chuỗi trong fixture là **dữ liệu của xe**, không phải phụ thuộc mã.
 */
class WmParseTest {
