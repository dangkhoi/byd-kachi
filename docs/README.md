...
> **Trạng thái**: Current · **Cập nhật**: 2026-09-09 · **Mục đích**: Bản đồ MỌI tài liệu hiện hành theo 9-loại taxonomy (R4). Không có trong index = archive/stale, KHÔNG authoritative (R0).
...
| [`specs/kachi-action-macros.html`](specs/kachi-action-macros.html) | **Gói 3 — GÓI LỆNH (một nút chạy nhiều việc)** (W2): lớp gộp lệnh thuần ở `:core` (thứ tự · chờ giữa bước · một bước hỏng không chết cả gói · mức bằng chứng = THẤP NHẤT trong các bước) + 4 gói định sẵn (mở/đóng hết kính từ 4 nút **đã chạy trên xe** thay vì nút gộp chưa kiểm · mở cửa+đèn · rời xe). [ĐO] 3 sự thật: **không có khái niệm gộp lệnh nào trong code** · nút nhãn **"Kính 50%" KHÔNG làm 50%** (ghi đúng cùng lệnh với kính cửa lái) · **đọc được** độ mở kính theo % nhưng **không có lệnh ghi** % ⇒ "kính ½ tất cả" là việc TRÊN XE. Trong 7 hành động owner nêu: 2 khỏi làm · 2 làm được · 3 chờ xe | Draft — chờ owner duyệt (4 OQ) | 2026-09-11 |
| [`specs/kachi-unified-capability-tile.html`](specs/kachi-unified-capability-tile.html) | **Gói 2 — Ô khả năng HỢP NHẤT + bảng lốp + tự lấy gió trong** (RW0 · W4 · W3): gộp "thông tin ĐỌC" và "HÀNH ĐỘNG ghi" về MỘT loại ô đặt được ở 3 vùng (thanh trên · ô giữa màn · thanh nút), realtime. Nền [ĐO] 4 sự thật: 195 mã của 3 bộ đăng ký KHÔNG xung đột (⇒ khỏi chuyển đổi dữ liệu đã lưu) · ô giữa màn ĐÃ nhận datum đọc · cổng chặn thật ở `DockConfig.setEnabled` · thanh nút CHƯA nhận `CarStatus`. Kèm bộ vẽ kiểu BOARD (chưa từng có) cho bảng 4 bánh + ô tick lấy gió bám mẫu ghế/lọc-bụi. ⚠ lệnh lấy gió tier OVERDRIVE = CHƯA kiểm trên xe | Draft — chờ owner duyệt (4 OQ) | 2026-09-10 |
