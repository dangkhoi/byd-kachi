...
>
> **🚀 2026-09-10 — ĐÃ PUSH LÊN `github.com/dangkhoi/byd-kachi` (PUBLIC, MIT).** Toàn bộ lịch sử Kachi (fork + P1–P5 + ARCH B0–B6 + Stage 7 SDD) đã lên remote; senior review APPROVED (full 5-module **2202/0**). Danh tính scrub (token công việc cũ) → **dangkhoi** ở cả content LẪN history (`git-filter-repo`) ⇒ **mọi SHA cũ ghi trong backlog (`4918ead`/`38fdb25`/`493b150`…) ĐÃ ĐỔI** (HEAD mới `508e512`); "CHƯA push" ở các dòng dưới nay đọc là **đã push**. Còn nợ: verify **trên xe** (ARCH-🚗 · X1 cast-side · P2/P3/P4 🚗). **W1** off-car DONE + **committed `b2c85f7`** (local, ahead-1, CHƯA push) — nối dữ liệu/điều khiển BydHal thật thay DemoCarData/NoCar; còn 🚗 verify trên xe + on-car grab-list (5 NEEDS_CAR). **2026-09-10 (chiều)**: test emulator (Play Store + mở-app-vào-ô) → lộ 1 bug launcher host-app-lúc-mở (P-bug2, diff sẵn) + giới hạn display phụ (D-emu); xem `docs/_handoff/session-2026-09-10-emulator-launcher-hosting.md` + `docs/diagnostics/kachi-emulator-app-hosting-2026-09-10.md`.
...
| ID | Việc | Làm | Xe | Ghi chú |
|----|------|-----|-----|---------|
| W1 | **Nối dữ liệu + điều khiển xe THẬT** (thay dữ liệu mẫu) — lớp khả năng ở `:core` (**121 datum / 8 nhóm** + **65 nút / 5 kiểu**) + cầu HAL ở `:app` + UI render theo kiểu tile | ✅ off-car | 🚗 | **[DONE 2026-09-10]** senior APPROVED · full 5-module **2302/0** · APK debug sạch. **Gate an toàn ĐÃ BỎ** (owner "tự dùng tự chịu"). **118/123 datum đã nối giá trị**; còn **5 chờ xe** (4 GPS + mục tiêu sạc — không có đường đọc) → danh sách lấy trên xe. Commit `b2c85f7` (local, **chưa push**) |
| **RW0** | **Ô khả năng HỢP NHẤT** — một tile chung render được cả READ (không bấm) lẫn WRITE (bấm), đặt ở thanh trên / widget / thanh tác vụ, cập nhật realtime | 🔲 | 🚗 | **NỀN cho W2·W4·W5** — làm trước thì 3 việc sau rẻ hẳn. Nguồn đọc = bản kê datum, nguồn action = bản kê nút; cùng một khung + đặt được 3 vùng |
| **W2** | **Đủ hoá thanh action + lớp GỘP LỆNH (scene/macro)** — 7 action owner nêu: cốp · đèn ban ngày · đèn đọc (tất cả) · kính ½ tất cả · kính mở hết · bảo trì gạt mưa · "mở cửa + tắt/mở đèn" | 🔨 | 🚗 | Nút đơn lẻ PHẦN LỚN đã có. **THIẾU**: (1) **lớp gộp lệnh** — 1 nút chạy nhiều lệnh (hiện chưa có trong code) ⇒ cần cho "mở cửa + tắt/mở đèn" và nghĩa "tất cả / ½ tất cả"; (2) **bảo trì gạt mưa** (nâng cần) — Dudu có, **chưa có mã số → lấy trên xe**; (3) preset **kính ½-tất-cả**; (4) xác nhận nút đèn đọc bật *tất cả* hay chỉ một |
| **W3** | **Tự lấy gió trong khi khởi động** — thêm ô tick ở Cài đặt / Tiện ích, nổ máy thì tự áp | 🔲 | 🚗 | Action **ĐÃ CÓ** (chế độ lấy gió, an toàn mọi lúc); chỉ cần ô tick + áp lúc khởi động (giống ghế và lọc bụi đang làm) |
| **W4** | **Widget áp suất lốp** — bảng 4 bánh (áp suất + nhiệt) | 🔨 | 🚗 | **DỮ LIỆU ĐÃ ĐỦ**: áp suất **cả 4 bánh** tier *đã kiểm* (đọc HAL thật) + nhiệt 4 bánh (chờ xe). CÒN: dựng widget bảng 4 bánh + đọc số thật trên xe |
...
| U1 | **Icon còn thiếu** — nút "Thanh"/"Cài đặt" đang là chữ; **~45 nút mới** (nóc/rèm/gương/đèn pha/ADAS/sạc…) đang dùng icon tạm | 🔲 | — | [ĐO] 2026-09-10: 8 nút mặc định CÓ icon riêng; ~45 nút mới xài icon tạm |
