...
- **Cài lần đầu**: tải `apk/Kachi-2.72-release.apk` (nút Raw/Download trên GitHub `dangkhoi/byd-kachi`, nhánh `main`) → chép vào xe → cài bằng **adb** (`adb install -r Kachi-2.72-release.apk`). ⚠ Chép vào xe rồi **tap** để cài có thể bị ROM DiLink báo *"Fail in installation of desktop apps"* (app là launcher) — cài bằng adb thì qua.
...
- **Xoay video** (*Cài đặt › Tiện nghi xe*, hai hàng *Xi-nhan trái: xoay video* / *Xi-nhan phải: xoay video*, từ 2.71): mỗi bên chọn riêng *Không xoay* / *↺ 90°* / *↻ 90°* / *180°* — mặc định trái ↺ 90°, phải ↻ 90° (hình gương cắt từ camera 360 vốn nằm ngang). Bên nào còn ngang hay lộn đầu thì chỉnh đúng hàng bên đó; hai bên không ảnh hưởng nhau.
...
### 1. First install + OTA
- **First install**: download `apk/Kachi-2.72-release.apk` from GitHub `dangkhoi/byd-kachi` (`main`), copy to the car and install with **adb** (`adb install -r …`). Tapping the APK on the head unit may fail with *"Fail in installation of desktop apps"* (it is a launcher); adb bypasses that gate.
...
- **Rotate video** (*Settings › Car comfort*, two rows *Left signal: rotate video* / *Right signal: rotate video*, since 2.71): pick *No rotation* / *↺ 90°* / *↻ 90°* / *180°* per side — defaults left ↺ 90°, right ↻ 90° (the mirror crop of the 360 camera is sideways). If one side is still sideways or upside down, adjust that side's row only; the two sides are independent.
