...
- **Cài lần đầu**: tải `apk/Kachi-2.69-release.apk` (nút Raw/Download trên GitHub `dangkhoi/byd-kachi`, nhánh `main`) → chép vào xe → cài bằng **adb** (`adb install -r Kachi-2.69-release.apk`). ⚠ Chép vào xe rồi **tap** để cài có thể bị ROM DiLink báo *"Fail in installation of desktop apps"* (app là launcher) — cài bằng adb thì qua.
...
### 1. First install + OTA
- **First install**: download `apk/Kachi-2.69-release.apk` from GitHub `dangkhoi/byd-kachi` (`main`), copy to the car and install with **adb** (`adb install -r …`). Tapping the APK on the head unit may fail with *"Fail in installation of desktop apps"* (it is a launcher); adb bypasses that gate.
